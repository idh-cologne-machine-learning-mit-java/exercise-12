...
public class Titanic {
	...
	public static void main(String[] args) throws Exception {
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
				.seed(seed).activation(Activation.TANH).list()
				.layer(new DenseLayer.Builder().nIn(numInputs).nOut(numInputs*2).build())
				.layer(new DenseLayer.Builder().nIn(numInputs*2).nOut(numInputs/2).build())
				.layer(new OutputLayer.Builder(LossFunctions.LossFunction.NEGATIVELOGLIKELIHOOD).activation(Activation.SOFTMAX)
						.nIn(numInputs/2).nOut(outputNum).build())
				.build();
